...
def note_to_pitch(midi_note):
    return 440.0 * pow(2, (midi_note-69)/12)
...
def parse_instrument(instrument):
    ...
    for bag in instrument.bags:
        ...
        pitch_correction = 48000/bag.sample.sample_rate  # sample rate correction
        sample_uuid = md5sum ( f"{instrument_name}_{bag.sample.name}_{number_to_note(root_note)}")
        filename =             f"{instrument_name}_{bag.sample.name}_{number_to_note(root_note)}_{sample_uuid}.wav"


        SampledSound = {   #the only data required for SampledSound xml part generation

            'SampledSound':{
                '@sampleIndex':   sample_index,
                '@name':          bag.sample.name,
                '@loopStart':     bag.cooked_loop_start if bag.sample_loop !=None  else 0,
                '@loopEnd':       bag.cooked_loop_end   if bag.sample_loop !=None  else 0,
                '@pitch':         pitch_correction * note_to_pitch(root_note) ,
                '@filename':      filename,
        ...
        sample_data_wide = {
            ...
            'root_note':          root_note,  # used to assign sample to note. base_note (root note override) might be absent, in this case default sample's pitch kicks in.
            'root_note_text':     number_to_note(root_note),  # used to name a sample file 
            ...
            'pitch_correction_coefficient': pitch_correction ,
            'pitch_uncorrected': note_to_pitch(root_note),
            'pitch': pitch_correction * note_to_pitch(root_note),
            'sampled_sound_line' :  SampledSound
        }


        sample_index += 1
        parsed_instrument.append(SampledSound)
         
      #  print(xml)
    return( {'SampleSet': parsed_instrument} )
        #print (sample_data)
...
with open(filepath, 'rb') as sf2_file:
    ...
    for instrument in sf2.instruments:
        ...
        a = (parse_instrument(instrument))
        print (a)
        #xml = (xmltodict.unparse(a,attr_prefix="@", pretty=True))
        #print(xml)
