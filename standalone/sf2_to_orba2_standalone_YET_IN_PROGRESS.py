...
def parse_instrument_samples(instrument: Sf2Instrument, preset: OrbaPreset.PresetEntry):
    ...
    print (f"Parsing instrument: { preset.name} #{instrument.bag_size}     {instrument.bag_idx}")   
    ...
    for bag in instrument.bags:
        ...
        instrument_name = slugify.slugify(preset.name, regex_pattern=allowed_pattern, lowercase=False)  # this name is used as a folder name, it should be valid for filesystem
...
def generate_OrbaPreset(raw_instrument_name):
        ...
        preset.name =f"SF2_{slugify.slugify(raw_instrument_name, lowercase=False)}_{preset.mode}"
        preset.uuid = md5sum(preset.name) 
        preset.sound_preset.name =   preset.name
        preset.sound_preset.sample_set.name =  preset.name
        preset.description = f"Generated with Sf2toOrba2 from SF2 [{sf2_filename}]:[{raw_instrument_name}] using  [{preset.mode}] artipreset template  [{template_name}]"
...
def mix_sampled_sounds(sampled_sounds):
...
def generateSampleSetMetadata(mixed_sampled_sounds):
    ...
    for i in velocity_thresholds_list:
     try:
        ...
        print (i)
        if note_thresholds_counter  != len(note_thresholds_list):
...
def check_for_skip_instrument(raw_instrument_name):

    # in case if we need to export specific instrument
    if  search_for_instrument.lower()  in raw_instrument_name  or  search_for_instrument== "":
        pass
    else:
        return True

    #skip "fake"  sf2 instrument, which is actually an End Of Instruments marker.
    if raw_instrument_name == 'EOI':
        return True
...
with open(sf2_filepath, 'rb') as sf2_file:
    ...
    preset_template = OrbaPreset.PresetEntry()
    preset_template.load_from_file(template_file)
    print(f"loaded template {preset_template.name} from {template_file}")
    

    #load sf2 data using sf2util library to parse sf2 bank
    ...
    for instrument in sf2.instruments:
        ...
        if check_for_skip_instrument(instrument.name): 
            continue
        
        # load template artipreset and fill some basic fields. More complex stuff will be modifed later.
        preset = generate_OrbaPreset(instrument.name)

        # create stub png picture
        image_filename=add_image_to_preset(preset)
        ...
        for s in preset.sound_preset.sample_set.sampled_sound:           
            ...
            else:   # some debug stuff, instead of sounds, exports speech generated names of the sample files.
                language = 'ru'
